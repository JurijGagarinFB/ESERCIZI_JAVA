...
public class Main {
    public static void main(String[] args) {
        ...
        String[] opzioni = {"Menu", "1 Genera Random", "2 Visualizzazione", "3 Ricerca", "4 Cancella numero con decremento", "5 Cancella numero e ritorna nuovo vettore", "6 Randomizza posizione numeri", "7 Raddoppia", "8 Fine"};
    ...
    public static int[] randomizzaPosizioneNumeri(int[] nEstratti) {
        int[] nuovoVettore = new int[nEstratti.length];

        // Copia degli elementi dall'array originale al nuovo array
        for (int i = 0; i < nEstratti.length; i++) {
            nuovoVettore[i] = nEstratti[i];
        }

        Random random = new Random();

        for (int i = nuovoVettore.length - 1; i > 0; i--) {
            // Seleziona un indice casuale da 0 a i
            int j = random.nextInt(0, i + 1);

            // Scambia l'elemento all'indice i con quello all'indice j
            int temp = nuovoVettore[i];
            nuovoVettore[i] = nuovoVettore[j];
            nuovoVettore[j] = temp;
        }

        return nuovoVettore; // Restituisce l'array con gli elementi randomizzati
    }

    public static int[] raddoppiaVettore(int[] nEstratti) {
        int[] nuovoVettore = new int[nEstratti.length * 2];
        int indice = 0; // Indice per il nuovo array
        for (int i = 0; i < nEstratti.length; i++) {
            nuovoVettore[indice] = nEstratti[i];
            indice++;
